...
from output import JSONWriter, VariableFilter, ParagraphVariablesMapper
...
def main():
    ...
    subcommands = ['analyze', 'filter-by-variable', 'analyze-and-filter', 'paragraph-variables-map']
...
Commands:
  ...
  paragraph-variables-map  Map paragraphs to variables that may change
...
For more information on a command, use: %(prog)s <command> --help
    ...
    if args.command in ("analyze", "analyze-and-filter", "paragraph-variables-map") and getattr(args, 'verbose', False) and args.quiet:
