The following prompt has been generated by Claude while working on a downstream demo project.
The demo project has a requirement for cobol-ast-parser and generated this prompt for Claude to implement it in this project.
...
A downstream project builds LLM prompts from COBOL execution traces. The prompt includes DATA DIVISION sections (WORKING-STORAGE + LINKAGE) for every executed module. For a typical execution with 26 modules, the full DATA DIVISION content is ~1.4 MB — too large for LLM context windows. We need to filter it down to only the variable groups actually referenced by the executed paragraphs.
...
The `variable_index` uses paragraph names as they appear in the analysis output (e.g., `"MAIN"`, `"INITIAL-OPERATIONS"`, `"PROCESS-HOST-ACTION"`). The caller passes the same names. Matching should be **case-insensitive** since COBOL is case-insensitive.
...
The downstream project will call this function like:

```python
from cobol_ast.api import filter_data_division_by_paragraphs, FilterOptions

result = filter_data_division_by_paragraphs(
    source_path=Path("MAINPROG.cbl"),
    paragraph_names=["MAIN", "INITIAL-OPERATIONS", "PROCESS-HOST-ACTION"],
...
the current implementation only returns filtered DATA DIVISION data. Here's what FilteredDataDivisionResult contains:
  ...
  scope correct for the downstream project's use case — just the filtered DATA DIVISION text?
